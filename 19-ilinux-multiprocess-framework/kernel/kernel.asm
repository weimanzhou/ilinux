...
%macro hwint_slave 1
	...
	; 1. 屏蔽当前中断
	in al, INT_S_CTLMASK	; 取出次 8259 的屏蔽位图
	or al, (1 << %1 - 8)	; 将该中断的屏蔽位置置位，表示屏蔽它
	out INT_S_CTLMASK, al	; 输出新的屏蔽位图，表示屏蔽当前中断
	...
	in al, INT_S_CTLMASK	; 取出 8259 屏蔽位图
	and al, ~(1 << %1 - 8)	; 将该位置0，启用中断
	out INT_S_CTLMASK, al	; 输出新的屏蔽位图，启用当前中断
	sti						; 打开中断
.0:	
	ret
%endmacro
...
; ---------------------------------------------------------------------
p_restart:
	mov esp, [curr_proc]		; 离开内核栈，esp -> 进程的栈帧结构
